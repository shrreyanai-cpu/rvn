...
export function registerAuthRoutes(app: Express): void {
  ...
  app.post("/api/auth/verify-otp", async (req, res) => {
    try {
      ...
      const activeOtps = await db.select().from(emailVerifications)
        .where(and(
      ...
      if ((activeOtp.attempts || 0) >= MAX_OTP_ATTEMPTS) {
        await db.update(emailVerifications).set({ used: true }).where(eq(emailVerifications.id, activeOtp.id));
        return res.status(429).json({ message: "Too many attempts. Please request a new code." });
      }

      if (activeOtp.otp !== otp) {
        await db.update(emailVerifications).set({ attempts: (activeOtp.attempts || 0) + 1 }).where(eq(emailVerifications.id, activeOtp.id));
